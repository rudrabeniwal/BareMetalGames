...
; set video mode to 320x200 256-colour
mov ax, 0x13
int 0x10
call init_game
game_loop:
    ...
    jmp game_loop
init_game:
    ;initialise paddle and ball positions
...
draw_paddle:
    ...
    mov cx, 10
    .draw_segment:
        mov [es:di], byte 0x0F      ; es:di points to the start of video memory plus an offset.
                                    ; bx*320 calculates the y-coordinate offset, as each row is 320 pixels wide.
                                    ; es:di+bx*320 gives the exact memory address for the pixel at (10, bx).
                                    ; byte 0x0F sets the color of the pixel to 0x0F (typically white or a bright color in 256-color mode).
                                    ; adds 320 to DI, moving the pointer down by one row. This is because the screen width is 320 pixels.
                                    ; decrements CX and repeats the loop until CX reaches zero. The loop will run 10 times, drawing 10 vertical segments of the paddle.
        add di, 320
        loop .draw_segment
    ret
...
update_ball:
    ; update ball positionn and check for collisions
    mov ax, [ball_x]
    add ax, [ball_dx]
    mov [ball_x], ax
    mov bx, [ball_y]
